...
package com.mylibrary.ui.showcase;
...
import com.mylibrary.ui.showcase.shapes.Circle;
import com.mylibrary.ui.showcase.shapes.RoundRect;
...
//                dismiss();
    ...
    public static class ViewActions {
        ...
        private void addCircleLeftOnView(float additionalRadiusRatio, int paddingLeft) {
            Rect rect = new Rect();
            view.getGlobalVisibleRect(rect);
            int cx = rect.centerX();
            int cy = rect.centerY() - getStatusBarOffset();
            int radius = (int) (Math.max(rect.width(), rect.height()) / 2f * additionalRadiusRatio);
            Circle circle = new Circle(cx - paddingLeft, cy, radius);
            circle.setDisplayBorder(settings.withBorder);
            tutoShowcase.tutoView.addCircle(circle);
            addClickableView(rect, settings.onClickListener, additionalRadiusRatio);
            tutoShowcase.tutoView.postInvalidate();
        ...
        public ShapeViewActionsEditor addCircleLeft(final float additionalRadiusRatio, final int paddingLeft) {
            view.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                @Override
                public boolean onPreDraw() {
                    addCircleLeftOnView(additionalRadiusRatio, paddingLeft);
                    view.getViewTreeObserver().removeOnPreDrawListener(this);
                    return false;
                }
            });
